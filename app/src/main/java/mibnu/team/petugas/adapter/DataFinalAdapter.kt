...
import mibnu.team.petugas.view.DetailDataValidActivity

class DataFinalAdapter (private var dataIII:MutableList<Data>, private var context: Context):RecyclerView.Adapter<DataFinalAdapter.ViewHolder>(){
    ...
    override fun getItemCount()=dataIII.size

    override fun onBindViewHolder(holder:ViewHolder, position: Int) = holder.bind(dataIII[position],context)
    ...
    class ViewHolder(itemView: View):RecyclerView.ViewHolder(itemView) {
        fun bind(d : Data, context: Context){
            with(itemView){
                ...
                setOnClickListener {
                    context.startActivity(Intent(context, DetailDataValidActivity::class.java).apply {
                        putExtra("DATA", d)
                    })
                }
            }
